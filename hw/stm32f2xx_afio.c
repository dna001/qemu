                                            ...
                                            uint32_t new_value, bool init)
{
    ...
    for(i = 0; i < AFIO_EXTI_PER_CR; i++) {
        ...
        if(!init) {
            ...
            stm32f2xx_exti_reset_gpio(s->stm32_exti, exti_line, STM32_GPIO_PERIPH_FROM_INDEX(old_gpio_index));
        }
        new_gpio_index = (new_value >> start) & 0xf;
        stm32f2xx_exti_set_gpio(s->stm32_exti, exti_line, STM32_GPIO_PERIPH_FROM_INDEX(new_gpio_index));
    }
                          ...
                          unsigned size)
{
    ...
    stm32f2xx_rcc_check_periph_clk((Stm32Rcc *)s->stm32_rcc, STM32_AFIO);
...
uint32_t stm32f2xx_afio_get_periph_map(Stm32Afio *s, stm32_periph_t periph)
{
    switch(periph) {
        case STM32_UART1:
...
static Property stm32_afio_properties[] = {
    DEFINE_PROP_PTR("stm32f2xx_rcc", Stm32Afio, stm32_rcc_prop),
    DEFINE_PROP_PTR("stm32f2xx_exti", Stm32Afio, stm32_exti_prop),
